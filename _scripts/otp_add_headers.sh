...
_add_canonical() {
    ...
    for file in "$@"; do
        ...
        CANONICAL_GREP=$(grep -P "<link rel=\"canonical\"" "${file}" || echo "")
        if [ "$(echo "${CANONICAL_GREP}" | wc -l)" -gt "0" ]; then
            sed 's@<link rel="canonical" href="[^"]*" />@<link rel="canonical" href="'"${CANONICAL_URL}${canonical}"'" />@g' -i -- "${file}"
            ADD_CANONICAL=false
        fi
...
for MAJOR_VSN in ${MAJOR_VSNS}; do
    ...
    _add_head_tags $(find "${TARGET_DIR}" -type f -name "*.html" -print0 | xargs -0 echo)
